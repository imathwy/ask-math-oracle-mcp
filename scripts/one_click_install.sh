...
usage() {
  cat <<'EOF'
Usage:
  ./scripts/one_click_install.sh [options] [-- quickstart-options]
...
Examples:
  ./scripts/one_click_install.sh --google-key "$GOOGLE_API_KEY"
  ./scripts/one_click_install.sh --repo-url https://github.com/imathwy/ask-math-oracle-mcp.git --openai-key "$OPENAI_API_KEY"
  ASK_MATH_ORACLE_REPO_URL=/path/to/local/repo ./scripts/one_click_install.sh --help
EOF
}
...
./scripts/quickstart.sh "${quickstart_args[@]}"
